...
# Importing all the EPD Script Logic Files
from .scriptSRC.ORG_EPD_Coolkidz import function_for_EPD_coolkidz
...
def Coolkidz_AR_ProcessRenderFile(request):
    return render(request, "AR_Coolkidz/AR_Process_template.html")
def Coolkidz_EPD_ProcessRenderFile(request):
    return render(request, "AR_Coolkidz/EPD_Process_template.html")
        ...
        try:
            ...
            if saved_file_path1 and saved_file_path2:
                ...
                if zip_file_path:
                    ...
                    logger.error('Failed to process file: No result ZIP path')
                    return JsonResponse({'error': 'Failed to process file'}, status=500)
            else:
                return JsonResponse({'error': 'Failed to save file'}, status=500)

        except Exception as e:
            logger.exception("Error processing file in generate_file_view_ar")
            return JsonResponse({'error': str(e)}, status=500)
...
# Adding and creating all the cores of EPD Files
@csrf_exempt
def generate_file_EPD(request):
    if request.method == 'POST' and request.FILES.get('excelFile'):
        excel_file = request.FILES['excelFile']
        action = request.POST.get('action')
        file_type, _ = mimetypes.guess_type(excel_file.name)
        ...
        try:
            ...
            if saved_file_path:
                # Process the file based on the action
                print(f"______________________________________________{action}______________________________________________")
                if action == 'Coolkids_EPD':
                    result_path = function_for_EPD_coolkidz(saved_file_path)
                else:
                    return JsonResponse({'error': 'Invalid action. Please select a valid action.'}, status=400)
...
def handle_uploaded_file_EPD(file):
    try:
        upload_dir = 'uploaded_EPD_Files'
        if not os.path.exists(upload_dir):
            os.makedirs(upload_dir)
