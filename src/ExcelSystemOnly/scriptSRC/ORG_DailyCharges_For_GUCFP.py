...
# pathing = "GUCFP_Appt_06202024" 

import warnings
warnings.filterwarnings('ignore', category=pd.errors.SettingWithCopyWarning)
warnings.filterwarnings('ignore', category=FutureWarning)  

def function_for_Gwinnett(pathing):
    ...
    # This will automatically Generate and attach the Currect Date at the ending of the generated Excel File!!
    ...
    # Removing The `(123)` ID Present in the Patient Name Column......
    result["Patient Name"] = result["Patient Name"].apply(lambda x: x.split("(")[0])

    # To convert Into Excel Format. 
    # result.to_excel(f"{pathing}.xlsx", index=0)
    # return result
    # return result.to_excel('UI\Results\GUCFP\[KUNJ]_GUCFP.xlsx')
    output_dir = 'Results/DailyCharges/Gwinnett'
    os.makedirs(output_dir, exist_ok=True)
    filename = os.path.basename(pathing)
    filename_no_ext = os.path.splitext(filename)[0]
    parts = filename_no_ext.split('_')
    kunjdate = parts[-1] 
    output_path = os.path.join(output_dir, f'Gwinnett_charges_of_{kunjdate}.xlsx')
